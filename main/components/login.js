...
function IsAuthenticated({auth,handleChange,handleSubmit,email,password,errored,errorText,theme}){
    ...
    let handleToggle = (e)=>{
        ...
        if(auth){
            return <div  id='loggedin' style={{color:theme.setColor}}>You are already logged in <Link to='/' style={{color:'green'}}>Back to main page</Link></div>
        }
        return (
            <form onSubmit={(e)=>handleSubmit(e)}  method='post' id='loginform' style={{backgroundColor:theme.setButtonColor,color:theme.setColor}}>
                ...
                <button onClick={(e)=>redirect('/signup')} className='user-link'style={{backgroundColor:theme.setBg,color:theme.setTextColor}}>New User?</button>
                <button onClick={(e)=>redirect('/reset')} className='user-link' style={{backgroundColor:theme.setBg,color:theme.setTextColor}}>Forgot Password?</button>
